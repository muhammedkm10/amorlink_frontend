...
function SubscriptionManagement() {
  ...
  const Fetchdata = async () => {
    const response = await admin_authentcatedApiClient.get(
      backendurls.subscription,
    )
  ...
  const AddingPlan = async () => {
    if (
      !addSubscription.plan_name ||
      !addSubscription.vlalidity_months ||
      ...
      try {
        const response = await admin_authentcatedApiClient.post(
          backendurls.subscription,
          addSubscription,
        )
  ...
  const EditsubmitHandler = async () => {
    if (
      ...
      !editSubscription.vlalidity_months ||
      !editSubscription.amount
    )
      Swal.fire({
        title: 'fill the form currectly',
        icon: 'error',
      ...
      try {
        const response = await admin_authentcatedApiClient.patch(
          `${backendurls.subscription}/${EditingSubscriptionId}/`,
          editSubscription,
        )
    ...
    <div>
      {!AddPage && !EditPage ? (
        ...
        <div className={`container ${styles.body}`}>
                ...
                <table className="table table-bordered table-striped">
                  ...
                  <tbody>
                    {subscriptionDetails.map((element) => (
                      <tr>
                        <td>
                          <p>{element?.plan_name}</p>
                        </td>
                        <td>
                          <p>{element.no_users ? element.no_users : '0'}</p>
                        </td>
                        <td>
                          <p>{element?.vlalidity_months}</p>
                        </td>
                        <td>
                          <p>{element?.amount}</p>
                        </td>
                        ...
                        <td>
                          <button
                            onClick={() => EditPlan(element)}
                            className="px-1 px-lg-5 m-2 bg-info border-0 text-white"
                          >
                            edit
                          </button>
                        </td>
          ...
          <button className={styles.add} onClick={() => setAddpage(true)}>
            <i className="fa fa-plus px-lg-2 px-1"></i>add plans
          </button>
        </div>
      ...
      AddPage ? (
        <div className={`text-center ${styles.addform}`}>
          ...
          <button
            type="submit"
            className="btn btn-primary mt-5 m-2"
            onClick={AddingPlan}
          >
            Submit
          </button>
          <button
            type="button"
            className="btn btn-secondary mt-5   m-2 ml-2"
        ...
        <div className={`text-center ${styles.addform}`}>
          ...
          <div className="form-group ">
            ...
            />
            <label
              htmlFor="listCheckbox"
              title="click to chang the list property"
            >
