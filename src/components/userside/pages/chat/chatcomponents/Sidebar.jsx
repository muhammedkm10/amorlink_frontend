    ...
    <div className={styles.sidebar}>
      ...
      {matches.length === 0 ? (
        <div className={styles.nomatches}>
          <p className={styles.findmatches}>
            you have no matches, find you matches
          </p>{' '}
          <Link
            to="/preferences"
            className={`text-decoration-none text-white ${styles.linkbutton}`}
          >
            preferences
          </Link>
        </div>
        ...
        <div className={styles.sidebarusers}>
          {matches.map((element) => (
            <Link
              className={styles.linktochat}
              to={`/chat/${user_id}/${element.id}`}
            >
              <div className={styles.user}>
                {' '}
                {!element.image_details ? (
                  <img
                  ...
                  <img
                    src={`${import.meta.env.VITE_IMAGE}${element.image_details}`}
                    className={styles.profilePhoto}
                    alt=""
                    width="40px"
                    height="40px"
