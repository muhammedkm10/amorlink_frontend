          ...
          {!isLoading ? (
            matches && matches.length === 0 ? (
              <p className="text-center text-white">
                Oops.....! There is no matches for you{' '}
                <Link to="/preferences">go to your preferences</Link>
              </p>
            ) : (
              matches.map((user) => (
                <div
                  className={`container col-lg-9 col-12 text-center ${styles.wrapper}`}
                >
                  <div className="row justify-content-evenly text-center">
                    <div
                      className={`col-lg-8 col-12 d-flex  align-items-center  ${styles.image}`}
                    >
                      {user.image_details ? (
                        <Link
                    ...
                    <div className={`col-lg-4 col-12 ${styles.details}`}>
                      <div className="d-flex justify-content-center align-items-center col-12 mt-lg-0 mt-3 ">
                        <Link to={`/chat/${current_user}/${user.id}`}>
                          {' '}
                          <button
                            className={`fa fa-comments fa-2x  ${styles.chat}`}
                            title="chat"
                          ></button>
                        </Link>
                        <button
                          onClick={() => RemoveFromMatches(user.id)}
