...
const AcceptRequest = async (userid) =>{
  const result = await Swal.fire({
    title: "do you wanto to accept the request?",
    icon: 'info',
                 ...
                 <p className='text-center text-white'>Oops.....! No requests for you <Link to="/preferences">go to your preferences</Link></p>
                   ...
                   requestedusers.map((user)=>(
                    ...
                    <div className={`col-lg-6 col-12 ${styles.details}`}>
                         ...
                         <Link ><button onClick={()=>AcceptRequest(user.id)} className={styles.button1}>accept</button></Link>
                         <Link ><button onClick={()=>RemoveFromRequests(user.id)}  className={styles.button2}>Reject</button></Link>
                         </div>
                    </div>
