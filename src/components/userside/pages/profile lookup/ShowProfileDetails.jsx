  ...
  return (
    <div>
      ...
      <div className={styles.fullbody}>
        <div className={`container-fluid  ${styles.background}`}></div>
        ...
        <div className={`container ${styles.head}`}>
          ...
          <div className={`row ${styles.content}`}>
            ...
            <div
              className={`col-lg-6 col-12 ${showSecondSide ? `${styles.secondside} show` : styles.secondside}`}
            >
              ...
              {(comingfrom === 'preferences' || comingfrom === undefined) &&
                (!subscribed ? (
                  <Link onClick={modalshowSubscribed} className="ms-2 ">
                    <button className={styles.button3}>
                      <i className="fas fa-lock me-2 text-warning"></i>Request
                      to match
                    </button>
                  </Link>
                ) : (
                  <Link onClick={matchRequestHandle} className="ms-2">
                    <button className={styles.button1}>Request to match</button>
                  </Link>
                ))}
              <br />
              <br />
              <a href="#other details" className={styles.otherdetailslink}>
                other details
              </a>
            </div>
          </div>
        ...
        <div id="other details" className={styles.otherdetails}>
              ...
              >
                ...
                {!subscribed ? (
                  <NavLink
                    ...
                    className={styles.links}
                  >
                    <CDBSidebarMenuItem icon="" onClick={modalshowSubscribed}>
                      <span>
          ...
          <div className={`container ${styles.showdetails}`}>
            ...
            {selectedItem === 'religion' && (
              <ReligionInformationLookup
                userid={lookupuserid}
                subscribed={subscribed}
              />
            )}
            {selectedItem === 'location' && (
              <LocationLookup userid={lookupuserid} subscribed={subscribed} />
            )}
            {selectedItem === 'family' && (
              <FamilyDetailsLookup
                userid={lookupuserid}
            ...
            {selectedItem === 'preference' && (
              <PartnerPreferencesLookup
                userid={lookupuserid}
                subscribed={subscribed}
              />
            )}
            {selectedItem === 'gallary' && (
              <PhotoGallaryLookup
                userid={lookupuserid}
                subscribed={subscribed}
              />
