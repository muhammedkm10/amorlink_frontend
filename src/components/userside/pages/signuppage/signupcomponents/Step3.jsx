...
function Step3({
  ...
  const height = [
    ...
    { label: '7\'0" and above (Over 210 cm)', value: '211-' },
  ]
  ...
  return (
    <div className="inputs text-center">
          ...
          {maritalStatusOptions.map((option) => (
            <div className="p-2 col-12 col-sm-6 col-md-3" key={option.value}>
              <RadioButton
                ...
                name="maritalStatus"
                value={option.value}
                checked={data.maritalStatus === option.value}
      ...
      {data.height ? (
        <div className="text-info p-0">Height : {data.height}</div>
      ) : (
        ''
      )}
      ...
      {data.familystatus ? (
        <div className="text-info p-0">Family status : {data.familystatus}</div>
      ) : (
        ''
      )}
    </div>
