...
function UpgradePlan({modalShowMethod,expiry_date,formatDate,user_plan_id}) {
 ...
 const CreatePaymentIntent =  async (validity_months) =>{
    ...
    if (!validity_months){
        ...
        try {
            ...
            const response = await authentcatedApiClient.post(`${backendurls.createintent}/${validity_months}`,{},{
                headers: {
       ...
       <div className={styles.modal}>
                <div className={styles.modal_content}>
                  ...
                  <div className={styles.package_details}>
                    <h6 className='p-2 '>Your plan will expire on : <span className=' text-warning'>{expiry_date}</span></h6>
                    {
                        newExpiryDate != null && 
                    <h6 className='p-2 '>New expiry date will be : <span className='text-warning'>{formatDate(newExpiryDate)}</span></h6>
                  ...
                  <label>
                    ...
                    <select  className={styles.dropdown} onChange={(e) => ChangeHandling(e)}>
                  ...
                  <button id={styles.fileinput} className={styles.modal_close} onClick={()=>modalShowMethod(false)}>close</button>
                  <button id={styles.fileinput} onClick={()=>CreatePaymentIntent(selectedOption)} className={styles.modal_close1}>pay for {selectedOption} months</button>
                  </div>
