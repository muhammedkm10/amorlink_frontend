...
import {
  admin_authentcatedApiClient,
  authentcatedApiClient,
  ...
  const fetchSubscriptionDetails = async () => {
    try {
      ...
      const response = await authentcatedApiClient.get(
        backendurls.user_subscription_details,
      )
  ...
  const paymentHandle = async (vlalidity_months) => {
    ...
    try {
      const response = await authentcatedApiClient.post(
        `${backendurls.createintent}/${vlalidity_months}`,
      )
      if (response.status == 200) {
    ...
    <div className={styles.fullbody}>
      ...
      <div className={`container  ${styles.subscription_details}`}>
        <h4 className="text-center text-white pt-4">
          Make your experiances good with our plans
        </h4>
        {loading ? (
          ...
          {!fetchloading ? (
            subscriptionPlans !== null ? (
              subscriptionPlans.map((plan) => (
                <div
                  className={` col-lg-3 col-12 ms-3 ${styles.firstplan}`}
                  key={plan.id}
                >
                  <div className={styles.heading}>
                    <h4>{plan.plan_name}</h4>
                  </div>
                  <h6 className="text-warning pt-2 text-center">
                    Validity : {plan.vlalidity_months} months{' '}
                  </h6>
                  <div className={styles.plandetails}>
                    <ul>
                      <li className="py-2">
                        Allow premium users to send unlimited messages to other
                        members
                      </li>
                      <li className="py-2">
                        {' '}
                        Facilitating better and more meaningful conversations.
                      </li>
                      <li className="py-2">
                        {' '}
                        Subscribed users can see more details about others.
                      </li>
                    </ul>
                  </div>

                  <button
                    onClick={() => paymentHandle(plan.vlalidity_months)}
              ...
              <div className={` col-12 ms-3 ${styles.firstplan}`}>
                <div className="row">
                  {UpgradePlanModal && (
                    <UpgradePlan
                      modalShowMethod={setUpgradePlanModal}
                      expiry_date={formatDate(
                        userSubscriptionDetails.expiry_date,
                      )}
                      formatDate={formatDate}
                      user_plan_id={userSubscriptionDetails.id}
                      ...
                      <tbody>
                        <tr className={styles.rows}>
                          ...
                          <td className={styles.info}>
                            <h5>{individualplan.plan_name}</h5>
                          </td>
                        </tr>
                        <tr className={styles.rows}>
                          ...
                          <td className={styles.info}>
                            {' '}
                            {formatDate(userSubscriptionDetails.date_started)}
                          </td>
                        </tr>
                        <tr className={styles.rows}>
                          <td className={styles.items}>Expiry date </td>
                          <td className={styles.info}>
                            {formatDate(userSubscriptionDetails.expiry_date)}
                          </td>
                        </tr>
                        <tr className={styles.rows}>
                          <td className={styles.items}>Amount paid</td>
                          <td className={styles.info}>
                            {individualplan.amount}
                          </td>
                        </tr>
                      </tbody>
                    </table>
                  ...
                  <div
                    className={`col-lg-6 text-center py-3 ${styles.aboutsubscription}`}
                  >
                    <h3 className="text-warning">
                      Upgrade Premium Subscription!{' '}
                    </h3>
                    <p className={styles.para_for_subscription}>
                      At AmorLink, we understand that finding a meaningful and
                      lasting relationship requires the right tools and
                      opportunities. While our free membership gives you access
                      to basic features, upgrading to our Premium Subscription
                      allows you to unlock the full potential of our platform,
                      including the ability to request matches with your ideal
                      partners.
                    </p>
                    <p className={styles.para_for_subscription}>
                      By upgrading to a Premium Subscription, you gain exclusive
                      benefits designed to enhance your journey towards finding
                      love:
                    </p>

                    <p className={styles.para_for_subscription}>
                      Don't miss out on the chance to make meaningful
                      connections. Upgrade to a Premium Subscription today and
                      start requesting matches with confidence!
                    </p>
